...
export const templates = [
  ...
  { group: 'Quality', phase: 'verify', title: 'Unit Test Plan & Evidence', code: 'TEST-UNIT-001', description: 'Kontrak perilaku unit, boundary mock, test case, command CI, dan bukti hasil test yang dapat direview.', sections: ['Scope & risk', 'Unit/behaviour map', 'Fixture & mock boundary', 'Test case matrix', 'CI gate & evidence'], tools: ['Vitest / Jest', 'GitHub Actions', 'Code coverage'], prompt: 'Buat Unit Test Plan siap eksekusi untuk [FITUR] di [NAMA PROYEK]. Petakan unit dan perilaku kritis, apa yang dimock dan mengapa, test case normal/boundary/error, fixture, command lokal/CI, quality gate, serta format evidence. Jangan menjadikan coverage sebagai satu-satunya bukti kualitas.' },
  { group: 'Quality', phase: 'verify', title: 'Integration Test Plan & Environment', code: 'TEST-INT-001', description: 'Uji batas antar service secara nyata dengan environment, data aman, setup/cleanup, kontrak, dan diagnostic artifact.', sections: ['System boundary', 'Dependencies & test data', 'Environment setup / teardown', 'Scenario matrix', 'Diagnostics & exit gate'], tools: ['Playwright', 'Postman / Bruno', 'Testcontainers'], prompt: 'Buat Integration Test Plan untuk [ALUR] di [NAMA PROYEK]. Identifikasi service/database/queue/vendor yang berinteraksi, pilih fake versus dependency nyata, desain data non-sensitif, setup/teardown, scenario sukses/gagal/retry/idempotency, observability yang dikumpulkan, serta release exit criteria.' },
  { group: 'Engineering', phase: 'build', title: 'Database Migration Plan', code: 'ENG-MIG-001', description: 'Rencana perubahan schema/data dengan strategi expand-migrate-contract, backup, validasi, cutover, dan rollback yang eksplisit.', sections: ['Change & compatibility', 'Pre-flight backup', 'Migration / backfill steps', 'Validation queries', 'Cutover & rollback decision'], tools: ['Prisma Migrate', 'Flyway', 'PostgreSQL'], prompt: 'Tulis Database Migration Plan untuk [PERUBAHAN]. Gunakan strategi expand-migrate-contract bila ada versi aplikasi lama. Sertakan dependency, lock/downtime risk, backup dan restore test, command ber-placeholder, validation query, sampling, cutover window, rollback point, data handling, owner, dan bukti yang harus dilampirkan.' },
  { group: 'Operations', phase: 'operate', title: 'Deployment Plan & Release Checklist', code: 'OPS-DEP-001', description: 'Rencana deploy yang dapat dijalankan: scope, owner, approval, urutan, progressive rollout, monitoring, komunikasi, dan sign-off.', sections: ['Release scope & owners', 'Readiness / approval gate', 'Deployment steps', 'Verification & monitoring', 'Communication & sign-off'], tools: ['GitHub Actions', 'Vercel', 'Sentry'], prompt: 'Buat Deployment Plan untuk release [VERSI] di [NAMA PROYEK]. Sertakan scope/known risk, change window, RACI, approval, version/artifact yang immutable, langkah deploy, feature flag atau canary, health checks dan metric threshold, komunikasi user/support, link rollback, dan post-deploy sign-off.' },
  { group: 'Operations', phase: 'operate', title: 'Rollback Plan & Drill Evidence', code: 'OPS-RBK-001', description: 'Langkah memulihkan known-good state untuk code, config, infrastructure, dan data; lengkap dengan trigger, decision owner, serta bukti drill.', sections: ['Trigger & decision authority', 'Known-good release', 'Code/config/infra/data recovery', 'Communication', 'Drill result & follow-up'], tools: ['GitHub Actions', 'Feature flags', 'Backup / restore'], prompt: 'Buat Rollback Plan yang dapat dipraktikkan untuk [RELEASE] di [NAMA PROYEK]. Definisikan trigger berbasis metric, siapa yang memutuskan, known-good version, urutan rollback code/config/infrastructure/database, strategi data jika migration sudah berjalan, komunikasi, verification, batas waktu, dan catatan hasil drill.' },
  { group: 'Quality', phase: 'verify', title: 'Test Evidence & Release Sign-off', code: 'QA-EVD-001', description: 'Rekaman test run, hasil, defect, exception/risk acceptance, dan keputusan release yang dapat diaudit.', sections: ['Release identity', 'Test execution record', 'Defect & exception register', 'Evidence links', 'Go / no-go sign-off'], tools: ['GitHub Actions', 'Jira / Linear', 'Test reports'], prompt: 'Buat Test Evidence & Release Sign-off untuk [RELEASE] di [NAMA PROYEK]. Rekam commit/build, environment, test run dan hasilnya, defect yang dibuka/diterima, evidence link, residual risk, keputusan go/no-go, approver, serta follow-up dengan owner dan tanggal.' },
...
export const artifactReferences = {
  ...
  'TEST-UNIT-001': { learn: ['Vitest mocking guide', 'https://vitest.dev/guide/mocking/functions'], visual: ['Template siap pakai: Unit Test Plan', '/examples/artifacts/unit-test-plan.md'] },
  'TEST-INT-001': { learn: ['Google: integration testing with real dependencies', 'https://google.github.io/building-secure-and-reliable-systems/raw/ch13.html'], visual: ['Template siap pakai: Integration Test Plan', '/examples/artifacts/integration-test-plan.md'] },
  'ENG-MIG-001': { learn: ['Microsoft: plan and test a database upgrade', 'https://learn.microsoft.com/en-us/sql/database-engine/install-windows/plan-and-test-the-database-engine-upgrade-plan?view=sql-server-ver17'], visual: ['Template siap pakai: Database Migration Plan', '/examples/artifacts/database-migration-plan.md'] },
  'OPS-DEP-001': { learn: ['GitHub deployment environments', 'https://docs.github.com/en/actions/concepts/workflows-and-actions/deployment-environments'], visual: ['Template siap pakai: Deployment Plan', '/examples/artifacts/deployment-plan.md'] },
  'OPS-RBK-001': { learn: ['AWS: plan for unsuccessful changes', 'https://docs.aws.amazon.com/wellarchitected/latest/operational-excellence-pillar/ops_mit_deploy_risks_plan_for_unsucessful_changes.html'], visual: ['Template siap pakai: Rollback Plan', '/examples/artifacts/rollback-plan.md'] },
  'QA-EVD-001': { learn: ['GitHub Actions workflow artifacts', 'https://docs.github.com/en/actions/concepts/workflows-and-actions'], visual: ['Template siap pakai: Test Evidence & Sign-off', '/examples/artifacts/test-evidence-release-signoff.md'] },
...
export const checklist = [
  ...
  },
  {
    ...
    tags: ['All'], template: 'Database Migration Plan'
  },
  {
  ...
  },
  {
    ...
    tags: ['All'], template: 'Test Strategy & UAT Script'
  },
  {
  ...
  },
  {
    ...
    tags: ['All'], template: 'Deployment Plan & Release Checklist'
  },
  {
